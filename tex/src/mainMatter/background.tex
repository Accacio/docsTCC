...
\Autoref{ex:loadingOfAWagon}.  The external events and firing condition
...
% , but its use of paths increases the
% efficiency for fault detection when compared with the latter articles.

This fault free observation is made by acquiring the observable signals
of the system (controller inputs and outputs) for a sufficiently long
period of time while the system works normally. Those signals can be seen on \autoref{fig:obsSignals}.
...
\begin{figure}[H]
  ...
  First, we assume the controller have $m_i$ binary inputs
  % , $i_h$, for $h=1,\dots,m_i$
  and $m_o$ binary outputs.
  % ,
  % $o_h$, for $h=1,\dots,m_o$,
  We create a Input\slash Output vector called $\mathbf{u}$ as follows:
...
\newcommand{\vu}{\mathbf{u}}
 ...
 % To reduce the size of the notation $\mathbf{u}(t)$ is going to be represented as $\vu_t$.
...
% So it leaves to the identification model the sole problem of reducing the language $L_{Exc}^{\leq n_0}$ and $L_{Exc}$ consequentially.
...
and the paths $p_1^2,p_2^2,p_3^2$ to \Autoref{alg:identification}, two models are identified, one for $k=1$ and another for
$k=2$.
