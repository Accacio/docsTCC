...
$s_i$ and $w_i$, and they are defined as:
\begin{align*}
  s_i&= \sigma_{i,1}\sigma_{i,2}\dots\sigma_{i,l_i-1} \\
  w_i&= \vu_{i,1}\vu_{i,2}\dots\vu_{i,l_i}
\end{align*}
...
be discarded, along with $s_i$ and $w_i$, since it does not present any new
...
sole problem of reducing the language $L_{Exc}$.
...
is presented on the form of modified paths, that are created using the paths
$p_i$ and a arbitrary variable $k$, a free parameter. The modified paths are
...
As an example of the computation, we can take the observation of the following
paths:
...
\end{align*}
If we choose $k=2$ and using \Autoref{eq:modifiedPath,eq:modifiedPathb} we can
