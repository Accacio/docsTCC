...
and when event $a$ happens it changes to state $z$ we can create a function
...
\todo{put diagram here 2 states x and y and explain state transition diagram    
have its alphaso we can create a function that represents a diagram called \emph{state transition diagram} can represent
graphically an automaton. 
We can begin by an example
}
\begin{figure}[H]
  \centering
...
\end{figure}
Now, a more complex example, from \cite{cassandras2009introduction}:
...
\begin{example}[Simple Automaton] ~\\
 ...
 \end{align*}
\todo{continuar} we can represent an automaton,
\end{example}
