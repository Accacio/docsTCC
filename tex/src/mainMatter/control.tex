...
represent the control of a system. This chapter will describe the logic of the
control of the system depicted in \Autoref{cha:system} and its design using
\CIPN{}. And after that the information needed to implement it in \LD.
...
transported by the belt to the identification part of the sorting unit. If the
piece is made of metal with an upwards concavity the piece continues in the belt until it
...
\verb|Storage Unit Horizontal Encoder| was placed just beside this same encoder, in order to
...
and because of the layout of the system this can
...
sake all transitions are represented in the Petri net as the transition $t_{156}$.
...
As said in other chapters, the implementation of the control in this work be implemented using \PLCs{}. The units shown in \Autoref{cha:system} are
