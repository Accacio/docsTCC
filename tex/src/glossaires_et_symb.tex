...
\newglossarystyle{dottedlocations}{%
 ...
 \quad \glossentrydesc{##1} \\~\\% the description
 % \space p. ##2\\
 % the number list in square brackets
 }%
...
\acr{NDAAO}{NDAAO}{Non-Deterministic Autonomous Automaton with Output}
...
\acr{SCL}{SCL}{Structured Control Language}
\acr{LD}{LD}{Ladder Diagram}
\acr{FBD}{FBD}{Function Block Diagram}
\acr{ST}{ST}{Structured Text}
\acr{IL}{IL}{Instruction List}
\acr{SFC}{SFC}{Sequential Function Chart}

\acr{CIPN}{CIPN}{Control Interpreted Petri Net}
\acr{CIPNs}{CIPNs}{Control Interpreted Petri Nets}

% DAOCT
\symbl{OmegaSet}{$\Omega$}{Set of IO vectors}
\symbl{SigmaSet}{$\Sigma$}{Set of events}
\symbl{XSet}{$X$}{Set of states}
\symbl{ffunction}{$f$}{Deterministic
  transition function}
\symbl{lambdafunction}{$\lambda$}{State
  output function}
\symbl{RSet}{$R$}{Set of path indices}
\symbl{thetafunction}{$\theta$}{Path
  estimation function}
\symbl{xZero}{$x_0$}{Initial State}
\symbl{XfSet}{$X_f$}{Set of final states}
%%% Local Variables:
%%% mode: latex
